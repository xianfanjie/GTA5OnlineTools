    ...
    public static void SpawnVehicle(string model, float z255, int dist, int[] mod)
    {
        ...
        Task.Run(() =>
        {
            ...
            for (int i = 0; i < 50; i++)
            {
                ...
                {
                    // 随机涂装
                    Hacks.WriteGA(Offsets.oVMCreate + 27 + 10 + 48, new Random().Next(0, mod[42] + 1));
                }
            }

            Hacks.WriteGA(Offsets.oVMCreate + 27 + 5, new Random().Next(0, 160));      // primary -1 auto 159  主色调
            Hacks.WriteGA(Offsets.oVMCreate + 27 + 6, new Random().Next(0, 160));      // secondary -1 auto 159  副色调
            Hacks.WriteGA(Offsets.oVMCreate + 27 + 7, new Random().Next(0, 160));      // pearlescent  珠光色

            Hacks.WriteGA(Offsets.oVMCreate + 27 + 69, -1);     // Wheel type  车轮类型
            Hacks.WriteGA(Offsets.oVMCreate + 27 + 33, -1);     // wheel selection  车轮选择
            Hacks.WriteGA(Offsets.oVMCreate + 27 + 8, new Random().Next(0, 160));      // wheel color  车轮颜色
            ...
            Hacks.WriteGA(Offsets.oVMCreate + 27 + 32, new Random().Next(0, 15));      // 氙气车灯 (0-14)
