...
namespace GTA5Heists.Views.Contract;
...
{
    private const int fixer_ratio = 262145 + 31955;     // joaat("FIXER_FINALE_LEADER_CASH_REWARD")
    private const int tuner_ratio = 262145 + 31249;     // joaat("TUNER_ROBBERY_LEADER_CASH_REWARD0"), &Global_262145.f_31249[0], true);
    ...
    private void Button_Read_Click(object sender, RoutedEventArgs e)
    {
        ...
        TextBox_TUNER_Value2.Text = Hacks.ReadGA<int>(tuner_ratio + 3).ToString();
        TextBox_TUNER_Value3.Text = Hacks.ReadGA<int>(tuner_ratio + 4).ToString();
        TextBox_TUNER_Value4.Text = Hacks.ReadGA<int>(tuner_ratio + 5).ToString();
    ...
    private void Button_Write_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            {
                Hacks.WriteGA(fixer_ratio, Convert.ToInt32(TextBox_FIXER_Value.Text.Trim()));
                Hacks.WriteGA(tuner_ratio + 1, Convert.ToInt32(TextBox_TUNER_Value0.Text.Trim()));
                Hacks.WriteGA(tuner_ratio + 2, Convert.ToInt32(TextBox_TUNER_Value1.Text.Trim()));
                Hacks.WriteGA(tuner_ratio + 3, Convert.ToInt32(TextBox_TUNER_Value2.Text.Trim()));
                Hacks.WriteGA(tuner_ratio + 4, Convert.ToInt32(TextBox_TUNER_Value3.Text.Trim()));
                Hacks.WriteGA(tuner_ratio + 5, Convert.ToInt32(TextBox_TUNER_Value4.Text.Trim()));
