...
public static class IniHelper
{
    ...
    /// <param name="key"></param>
    /// <returns></returns>
    public static bool ReadBoolValue(string section, string key)
    {
        var temp = new StringBuilder(1024);
        _ = GetPrivateProfileString(section, key, string.Empty, temp, temp.Capacity, IniPath);
        return temp.ToString().Contains("true");
    }
    ...
    /// <param name="value"></param>
    /// <param name="intPath"></param>
    public static void WriteBoolValue(string section, string key, bool value)
    {
        if (value)
        {
            WritePrivateProfileString(section, key, "true", IniPath);
        }
        else
        {
            WritePrivateProfileString(section, key, "false", IniPath);
        }
    }
